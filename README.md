...
agentprecept 的集成方式是"外挂层"——不修改任何现有 .py/.yaml 文件，全部产出在 AGENTS.md + docs/ 目录下。适合已有项目的渐进式引入，也适合新项目的零风险冷启动。
...
agentprecept 不是让 Agent 更强——**是给 Agent 一份施工图，给用户一份验收单**。你在家装时不懂水电，也能对照 checklist 说"这里不对"。技术上同时给工程师完整的 14 维审计框架和 8 阶段生产就绪退出标准。
...
| 监理检查工具——10 维自动化巡检 | **basic-audit.py** — 命名/引用/编号/残留/图格式/追溯/覆盖率/狗粮/声明校验/设计覆盖，一行命令全跑（--gate 开启后 2 维） |
...
**10 维审计**：`python scripts/basic-audit.py docs/ --gate` 一行命令跑完 10 维（命名/引用/编号/骨架/图格式/设计追溯/覆盖率/狗粮/声明校验/设计覆盖）。14 维审计框架中 10 维自动执行，4 维由 Agent/用户按需触发。
...
用户说"帮我写一个日记应用"、"搭一个 API 服务"——Agent 识别为新项目创建意图，自动：运行 `init.ps1`（AGENTS.md + 8 核心文档，含 MEMORY 自动生长）→ 提示 git init → 出架构设计草稿 → 等确认 → 才能写代码。骨架不可跳过，即使用户说"直接做"。
...
| `docs/MEMORY.md` | 持久偏好和项目约束（Agent 自动生长） |
| `docs/mcp-tools.md` | MCP Server 5 个 tool 参考 |
...
agentprecept init /your/project    # 一键生成核心文件
agentprecept sync                  # 从代码自动同步 project-graph
agentprecept audit --gate          # 10 维审计（命名/引用/编号/骨架/格式/追溯/覆盖/狗粮/声明/设计覆盖）
agentprecept doctor                # 诊断缺什么
agentprecept-mcp                         # 启动 MCP Server（6 tools: query/audit/diff/decision/handoff/design_gate）
```
...
1. 运行 init → AGENTS.md + 8 核心文档就位（含 MEMORY 自动生长）
...
│
├── agentprecept/     ← CLI（init/sync/audit/doctor）+ MCP Server（5 tools）
