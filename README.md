...
Agent 拿到需求容易跳过设计直接写代码。agent-compass 强制：新项目必须出架构设计（L2_D01），新模块必须有模块设计（L2_F07），API 变更必须先写契约（L2_G03）。设计文档没确认，Agent 不许动工。EXPLORE 下可简化但不可为零；PRECISE 下必须达模板完成标准。
...
| 文件 | 作用 |
|------|------|
| `AGENTS.md` | Agent 行为规则（Auto-Pilot + 设计先行 + 硬规则 + 双模式） |
