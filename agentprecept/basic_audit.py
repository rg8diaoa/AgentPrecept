"""agentprecept 审计脚本 — 15 维自动化审计

用法: python -m agentprecept.basic_audit docs/
      python scripts/basic-audit.py docs/         # 向后兼容 stub
      python scripts/basic-audit.py docs/ --gate   # 全维度
输出: 15 维审计报告（markdown）
"""
import os, re, sys
...
def check_readme_claims(docs_dir: str) -> list[dict]:
    ...
    claims = [
        ("15 维审计", r"(\d+)\s*维审计[^框架]", "agentprecept/basic_audit.py", "regex", r"^def check_", 15),
        ("6 个 MCP tool", r"(\d+)\s*个\s*MCP\s*tool", "agentprecept/mcp_server.py", "regex", r"@mcp\.tool", 6),
        ("6 维代码扫描", r"(\d+)\s*维代码扫描", "agentprecept/sync_graph.py", "regex", r"def build_.*_relations", 6),
