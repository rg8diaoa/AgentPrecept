...
│   └── 默认行为层（无条件执行）
│
├── CLI + MCP 层 (agent_compass/)      ← 用户和 Agent 的命令入口
│   ├── cli.py          — init / sync / audit / doctor 四个命令
│   └── mcp_server.py   — MCP Server（FastMCP，注册 5 个 Tool，0 个 Resource）
│
...
project-graph.yaml  ←──── basic-audit.py（审计）
    │                       │
    ├── Agent 直接读取        │  结构化报告
    └── MCP Agent 调用 ───────┘  (via mcp_server.py: query/audit/diff/decision/handoff)
         │
         ▼
    L4_O01 / HANDOFF / MEMORY / docs/

MCP 通道（Agent 不遍历文件系统，通过结构化 API 获取项目知识）:
  MCP Agent → mcp_server.py → scripts/basic-audit.py | scripts/sync-graph.py → 文件系统
```
