...
| 为什么 basic-audit 加图格式校验 | 格式兼容 | mindstream 的 project-graph 用 direction:import 而非标准 type:depends_on，sync-graph.py 无法识别。增加第 4 维审计检查 top-level 键/relations 字段/type 值，防止非标格式 |
