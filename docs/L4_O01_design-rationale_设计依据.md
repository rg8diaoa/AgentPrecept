...
| 为什么 project-graph 要 100% 覆盖所有项目文件 | 图即真相 | v0.3.0 的 structure 缺 SKILL.md/CHANGELOG/pyproject/.github 等 10+ 条目，relations 缺 ~10 条边。不全的图无法驱动涟漪分析和审计。v0.4.0 全量重建：根目录所有非构建文件全覆盖 |
| 为什么需要涟漪分析 | 图驱动的变更影响 | 世界模拟器 V7.0 已有涟漪分析器（🔴🟡🟢）。AgentPrecept 的 relations 天然支持——改了 X 文件→查图的 to 字段→输出 DIRECT/INDIRECT/SAME_PKG 三级影响。ripple_check.py 集成进 design_gate 和 audit |
| 为什么审计从线性 15 维重构为 4-scope | 按对象分类 | 线性维度列表在 15 维后已难维护——新增维度时容易遗忘旧维度。4-scope（docs/code/git/config）按审计对象自然分类，每个 scope 独立可运行（`--scope docs`），`--gate` 全跑 |
