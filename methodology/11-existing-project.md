...
> agent-compass 不要求你从零建文档体系。可以在已有项目上渐进接入。

---

## 不要一次全搬

最常犯的错误：clone agent-compass 后把 templates/ 全复制到项目里，然后看着 22 个 ⏳ 模板不知道从哪开始。
...
```
读取 src/ 目录结构，按 agent-compass 的三层图格式填充 docs/project-graph.yaml 的结构层。只列主要模块（不大于 10 个包），不要列到函数级别。
```
...
1. 复制 `templates/L4_O01_design-rationale_设计依据.md` 到 `docs/L4_O01_design-rationale.md`
2. 用以下 prompt：
